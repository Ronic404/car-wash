...
- Управление администраторами (страница «Сотрудники»: заявки на регистрацию, подтверждение/удаление)
...
- `main` — имеет доступ к странице **«Сотрудники»** и может подтверждать заявки/удалять админов.
- `regular` — обычный администратор без доступа к странице **«Сотрудники»**.

Регистрация администратора происходит через **заявку**: пользователь отправляет заявку, а `main` подтверждает её в админке.
Исключение: **самый первый администратор** в системе создаётся как `main` и активный (bootstrap).
